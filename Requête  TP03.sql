...
select COUNT(auteur_id) AS livre_avec_auteur FROM livres ;  
...
select user_id , COUNT(*) AS nb_emprunts	
FROM emprunts
GROUP BY user_id ; 


-- Genres ayant plus de 3 livres.
SELECT genre , COUNT(*) AS genre3livres 
FROM livres
GROUP BY genre 
HAVING COUNT(*) > 3 ;

-- Utilisateurs ayant au moins 2 emprunts.
SELECT user_id , COUNT(*) AS nb_utillisateur   
FROM emprunts  
GROUP BY user_id  
HAVING COUNT(*)>= 2 ;     

-- Année de publication moyenne des livres.
select AVG(annee_publication) as moyenne_annee FROM livres ;   

-- Livre le plus ancien et le plus récent (MIN/MAX).
select MIN(annee_publication) AS livre_ancien ,
 MAX(annee_publication) AS livre_jeune FROM livres ; 

-- Nombre d'emprunts par utilisateur avec son nom (JOIN + GROUP BY).
SELECT u.nom, COUNT(*) AS nb_emprunts
FROM emprunts e
