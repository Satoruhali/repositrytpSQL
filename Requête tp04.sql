...
-- Trouve les livres publiés après l'année moyenne de publication.
SELECT genre, annee_publication FROM livres
WHERE annee_publication > (SELECT AVG(annee_publication) FROM livres) ;  


-- Trouve les livres dont l'auteur a le même id que le premier auteur de la table.
SELECT titre FROM livres 
WHERE auteur_id = (SELECT MIN(id) FROM auteurs);

-- Trouve les utilisateurs qui ont au moins un emprunt.
SELECT * FROM utilisateurs   
WHERE id IN (SELECT DISTINCT user_id FROM emprunts ) ; 

-- Trouve les livres qui n'ont jamais été empruntés (sans LEFT JOIN, avec NOT IN).
SELECT titre FROM livres  
WHERE id NOT IN (SELECT livre_id FROM emprunts );


-- Simule un transfert d'emprunt : un utilisateur A rend un livre, utilisateur B l'emprunte.
-- Utilise BEGIN, les UPDATE, puis COMMIT.
-- Ajoute un ROLLBACK commenté pour montrer comment annuler.
